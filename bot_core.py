...
def guardar_estado_simulacion(state_file_path, posicion, precio_entrada, max_pnl_pct, pnl_acumulado, trades_totales, monto_invertido=6.0, rachas_perdidas=0, timestamp_entrada=0.0, confianza_ia=0.0):
    ...
    try:
        import uuid
        temp_file = f"{state_file_path}.{uuid.uuid4().hex}.tmp"
    ...
    def __init__(self, model_path, terminal_log_file_path):
        ...
        if os.path.exists(self.model_path):
            try:
                ...
                self.trained = True
                print(f"{Fore.GREEN}[IA] Modelo SOL cargado desde disco. Listo para operar.")
            except Exception as e:
...
class CerebroRL:
    ...
    def predecir_accion(self, data_dict, current_position, current_pnl_pct):
        ...
        vol_base = max(data_dict.get('vol_total', 0.0), 1.0)
        
        ofi = np.tanh(data_dict.get('ofi', 0.0) / vol_base)
        ofi_ema_5 = np.tanh(data_dict.get('ofi_ema_5', 0.0) / vol_base)
        ofi_ema_15 = np.tanh(data_dict.get('ofi_ema_15', 0.0) / vol_base)
        cvd = np.tanh(data_dict.get('cvd', 0.0) / (vol_base * 10.0))
        liq_longs = np.tanh(data_dict.get('liq_longs', 0.0) / 1000.0)
        liq_shorts = np.tanh(data_dict.get('liq_shorts', 0.0) / 1000.0)
...
# --- FILTRO DE EXPERIENCIA (XGBoost) ---
class FiltroExperienciaXGB:
    """
    Filtro secundario que evalua las senales del RL usando XGBoost
    entrenado sobre la base de datos cerebro_experiencia.db.
    """
    def __init__(self, model_path="filtro_xgb_sol.joblib"):
        self.model_path = model_path
        self.model = None
        self.trained = False
        ...
        try:
            ...
            if os.path.exists(self.model_path):
                ...
                self.trained = True
                print(f"{Fore.GREEN}[XGB] Filtro de experiencia cargado exitosamente.")
            else:
                print(f"{Fore.YELLOW}[XGB WARNING] No se encontro {self.model_path}. El filtro estara inactivo.")
        except Exception as e:
            print(f"{Fore.RED}[XGB ERROR] Error cargando filtro: {e}")

    def aprobar_trade(self, data_dict):
        """
        Retorna True si el filtro predice que el trade sera ganador (Clase 1).
        Retorna False si predice que sera perdedor (Clase 0) o si no hay modelo.
        """
        if not self.trained or self.model is None:
            return True # Si no hay filtro, aprobamos todo por defecto
...
class GestorExperiencia:
    ...
    def crear_tabla(self):
        ...
        ''')
        # Intentar añadir la columna por si la tabla ya existía
        try:
            self.cursor.execute("ALTER TABLE snapshots_rl ADD COLUMN vwap_dist REAL DEFAULT 0.0")
        ...
        try:
            self.cursor.execute('''
                INSERT INTO snapshots_rl (
                    ...
                    cvd, liq_longs, liq_shorts, ema_15m_dist, rsi_5m, macro_sentiment, vwap_dist
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                ...
                obs_dict.get('macro_sentiment', 0.0), obs_dict.get('vwap_dist', 0.0)
            ))
            self.conn.commit()
        except Exception as e:

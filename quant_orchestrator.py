...
def main():
    ...
    MODO_PRODUCCION = False # Cambiar a True para usar dinero real (bot_sol_live.py)
    ...
    if MODO_PRODUCCION:
        ...
        bot_name_label = "Bot SOL (LIVE/REAL)"
    else:
        bot_sol_PATH = os.path.join(BASE_DIR, "bot_sol", "bot_sol_paper.py")
        bot_name_label = "Bot SOL (Simulador)"
    ...
    bot_process = run_process(bot_sol_PATH, bot_name_label)
    ...
    try:
        while True:
            ...
            if bot_process.poll() is not None:
                print(f"{Fore.RED}[ORQUESTADOR] Bot SOL ha terminado. Reiniciando...")
                bot_process = run_process(bot_sol_PATH, bot_name_label)

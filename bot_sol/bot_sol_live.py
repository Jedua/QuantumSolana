...
PARENT_DIR_BOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
load_dotenv(os.path.join(PARENT_DIR_BOT, ".env"))
...
PAPER_MODEL_FILE = os.path.join(SCRIPT_DIR, "..", "modelo_rl_sol.zip")
...
async def main_loop(db, ia, exp, risk_manager):
    ...
    print(f"{Fore.MAGENTA} [SISTEMA] SOLANA BOT - LIVE TRADING (DINERO REAL)")
    print(f"{Fore.MAGENTA}=====================================================")
    ...
    log_terminal_event("INFO", "SYSTEM_START", "Iniciando simulacion SOL.", LIVE_TERMINAL_LOG_FILE)
    ...
    # --- SINCRONIZACION INICIAL CON BINANCE ---
    print(f"{Fore.YELLOW}[SISTEMA] Verificando posiciones abiertas en Binance al inicio...")
    try:
        pos_info = binance_client.futures_position_information(symbol=SYMBOL_WSS.upper())
        for p in pos_info:
            if p['symbol'] == SYMBOL_WSS.upper():
                real_amt = float(p['positionAmt'])
                if real_amt != 0.0:
                    real_entry = float(p['entryPrice'])
                    posicion = 'LONG' if real_amt > 0 else 'SHORT'
                    precio_entrada = real_entry
                    cantidad_comprada = abs(real_amt)
                    monto_invertido = (precio_entrada * cantidad_comprada) / LEVERAGE
                    print(f"{Fore.GREEN}[SISTEMA] Sincronizado exitosamente! Posicion adoptada: {posicion} {cantidad_comprada} SOL @ {precio_entrada}")
                else:
                    if posicion is not None:
                        print(f"{Fore.YELLOW}[WARNING] Discordancia inicial: Estado local tenia posicion, pero Binance esta en 0. Limpiando.")
                        posicion = None
                        cantidad_comprada = 0.0
                break
    except Exception as e:
        print(f"{Fore.RED}[SISTEMA] Error sincronizando estado inicial: {e}")
    ...
    # Conexion Multiplexada (Depth + AggTrade + Liquidations para SOL y BookTicker para BTC)
    ...
    async with websockets.connect(url, ping_interval=None) as ws:
        print(f"{Fore.GREEN}[ONLINE] BOT LIVE ACTIVO. CONECTADO A BINANCE FUTURES.")
        ...
        while True:
            try:
                ...
                if time.time() - last_sentiment_read > 5:
                    ...
                    macro_sentiment_score = await loop.run_in_executor(None, read_sentiment, SENTIMENT_FILE_PATH, LIVE_TERMINAL_LOG_FILE)
                    last_sentiment_read = time.time()

                # Actualizar MTF Context cada 60s
                if time.time() - last_mtf_update > 60:
                    loop = asyncio.get_event_loop()
                    mtf_res = await loop.run_in_executor(None, fetch_mtf_data, SYMBOL_WSS.upper(), LIVE_TERMINAL_LOG_FILE)
                ...
                # Sincronizacion con Binance cada 15s
                if time.time() - last_sync_update > 15:
                    loop = asyncio.get_event_loop()
                    def sync_api():
                        try:
                            pos = binance_client.futures_position_information(symbol=SYMBOL_WSS.upper())
                            for p in pos:
                                if p['symbol'] == SYMBOL_WSS.upper():
                                    return float(p['positionAmt'])
                            return 0.0
                        except: return None
                    real_amt = await loop.run_in_executor(None, sync_api)
                    if real_amt == 0.0 and posicion is not None:
                        print(f"\n{Fore.YELLOW}[WARNING] Sincronizando... Posicion cerrada externamente en Binance.")
                        posicion = None
                        cantidad_comprada = 0.0
                        timestamp_entrada = 0
                ...
                if '@aggTrade' in stream_name:
                    ...
                    # CVD Real acumulativo con decaimiento lento (memoria macro)
                    cvd_vol *= 0.9995
                    alpha_cvd = 2 / (100 + 1)
                    cvd_ema = alpha_cvd * cvd_vol + (1 - alpha_cvd) * cvd_ema

                if time.time() - ultimo_reporte > 300:
                    print(f"\n[ESTADO] PnL Neto Real: ${pnl_acumulado:.2f} | Trades: {trades_totales}")
                    ultimo_reporte = time.time()
                ...
                depth_vol_bid = np.sum(bids[:,1])
                depth_vol_ask = np.sum(asks[:,1])
                imbalance = (depth_vol_bid - depth_vol_ask) / (depth_vol_bid + depth_vol_ask)
                
                # Para OFI, usamos SOLO el Nivel 1 (Literatura estándar de microestructura)
                current_vol_bid = bids[0][1]
                current_vol_ask = asks[0][1]
                ...
                # --- CALCULO DE VWAP EN TIEMPO REAL ---
                vol_tick = current_vol_bid + current_vol_ask
                cumulative_pv += mid_price * vol_tick
                cumulative_v += vol_tick
                vwap = cumulative_pv / cumulative_v if cumulative_v > 0 else mid_price
                vwap_dist = (mid_price - vwap) / vwap
                ...
                if ia.trained and not estado_hibernacion:
                    ...
                    def cerrar_posicion(motivo="RL_CLOSE"):
                        nonlocal posicion, precio_entrada, max_pnl_pct, pnl_acumulado, trades_totales, monto_invertido, rachas_perdidas, cooldown_actual, ultimo_cierre, timestamp_entrada, cantidad_comprada
                        
                        
                        side = SIDE_SELL if posicion == 'LONG' else SIDE_BUY
                        if cantidad_comprada > 0:
                            exito, err_msg = ejecutar_orden_mercado(SYMBOL_WSS, side, cantidad_comprada, reduce_only=True)
                            if not exito:
                                if "ReduceOnly" in err_msg:
                                    print(f"{Fore.YELLOW}[SISTEMA] Posicion cerrada manualmente en app. Forzando sincronizacion.")
                                    posicion = None
                                    cantidad_comprada = 0.0
                                    timestamp_entrada = 0
                                    ultimo_cierre = time.time()
                                    return
                                print(f"{Fore.RED}[SISTEMA] Abortando cierre logico por fallo en Binance API.")
                                return # Abortar el cierre interno para reintentarlo
                            cantidad_comprada = 0.0
                        ...
                        pnl_neto_pct = pnl_pct - ROUND_TRIP_FEE  # PnL Neto % para el log
                        ...
                        registrar_trade_log(posicion, precio_entrada, precio_salida, pnl_neto_pct, pnl_neto_usd, LIVE_LOG_FILE, LIVE_TERMINAL_LOG_FILE)
                    ...
                    if action == 1:
                        ...
                        if posicion is None:
                            ...
                            if not exito:
                                cantidad_comprada = 0.0
                                if "Margin" in err_msg or "Margin is insufficient" in err_msg:
                                    print(f"{Fore.YELLOW}[SISTEMA] Margen insuficiente. Limpiando estado zombie.")
                                    posicion = None
                                else:
                                    print(f"{Fore.RED}[SISTEMA] Abortando apertura LONG interna por fallo en API.")
                                continue
                            posicion = 'LONG'
                        ...
                        if posicion is None:
                            ...
                            if not exito:
                                ...
                                if "Margin" in err_msg or "Margin is insufficient" in err_msg:
                                    ...
                                    print(f"{Fore.RED}[SISTEMA] Abortando apertura SHORT interna por fallo en API.")
                                continue
                            posicion = 'SHORT'
                            # --- RISK MANAGER DECIDE TAMANO POSICION ---
                    ...
                    if posicion is not None:
                        ...
                        # 3. Time-Decay (Caducidad de 45 min si está atascado sin ganancias superiores al 0.05%)
                        elif time.time() - timestamp_entrada > (45 * 60) and current_pnl_pct <= 0.0005:
                ...
                print(f"\n{Fore.RED}[ERROR] Loop Simulacion SOL: {repr(e)}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n[SISTEMA] Bot LIVE Detenido. Guardando datos finales...")
        db.close()
        exp.close()
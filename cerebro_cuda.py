...
def main():
    ...
    while True:
        try:
            ...
            # SOL
            res_eth = optimizar_moneda("SOL", DB_ETH)
            if res_eth: 
                current_config["SOL"] = res_eth
                ...
                # Reporte Visual
                p = current_config['SOL']
                print(f"SOL: TP {p['take_profit']*100:.2f}% | SL {p['stop_loss']*100:.2f}% | IMB {p['imbalance']} | IA_CONF {p.get('ia_confidence', 'N/A')} | OFI {p.get('ofi_threshold', 'N/A')} | OFI_EMA5 {p.get('ofi_ema_5_threshold', 'N/A')}")
